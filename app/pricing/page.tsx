...
import { cacheCurrentUser, getCurrentUser, getSessionUser, CurrentUser } from "@/lib/api";
import "./pricing.css";
...
export default function PricingPage() {
  ...
  const [sessionChecked, setSessionChecked] = useState(() => getSessionUser() !== null);
  ...
  return (
    <main className="pricing-page">
      <section className="pricing-hero">
        <div className="pricing-brand"><Image src="/logo.svg" alt="EchoStream" width={32} height={32} priority /><span>EchoStream AI</span></div>
        <div className="pricing-heading"><h1>Atmospheric Precision.</h1><h2>Choose your tier.</h2><p>Unlock the full potential of your AI workflow with tools designed for cinematic fidelity.</p></div>
        <div className="pricing-plans">
          {PLANS.map((plan) => {
            const cta = getCta(plan.name, user);
            return <article className={`pricing-card-wrap${plan.popular ? " pricing-card-popular" : ""}`} key={plan.name}>
              {plan.popular && <span className="pricing-badge">BEST VALUE</span>}
              <div className="pricing-card">
                <h3>{plan.name}</h3><div className="pricing-price"><strong>{plan.price}</strong><span>/mo</span></div>
                <p className="pricing-description">{plan.description}</p><div className="pricing-divider" />
                <ul>{plan.features.map((feature) => <li key={feature}><span className="pricing-check"><IonIcon icon={checkmark} aria-hidden="true" /></span><span>{feature}</span></li>)}</ul>
                <div className="pricing-button-wrap">
                  <Link href={cta.href} className={`pricing-button${plan.popular ? " pricing-button-primary" : ""}`} aria-disabled={!sessionChecked} tabIndex={!sessionChecked ? -1 : undefined}>
                    {!sessionChecked ? <span className="pricing-spinner" role="status" aria-label="Loading session" /> : <span>{cta.label}</span>}
