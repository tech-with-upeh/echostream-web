...
function formatPlan(plan: string) {
  return !plan ? "Starter" : plan.charAt(0).toUpperCase() + plan.slice(1).toLowerCase();
}
...
export default function DashboardPage() {
  ...
  const [subscriptionLoading, setSubscriptionLoading] = useState(() => !cachedSubscription && cachedUser?.plan?.toLowerCase() !== "starter");
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      try {
        ...
        // Starter users do not have subscription/payment management data.
        // Do not wait for /payments/manage for them.
  ...
  const endDate = subscription ? formatDate(subscription.subscription_ends_at) : formatDate(user?.subscription_ends_at ?? null);
        ...
        <div className="dashboard-overview">
          <div className="dashboard-plan-card">
            ...
            <div className="dashboard-plan-heading">
              ...
              {user || subscription ? <h1>{plan}</h1> : <span className="dashboard-plan-spinner" role="status" aria-label="Loading subscription" />}
            </div>
