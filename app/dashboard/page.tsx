...
const settingGroups = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        icon: "/logo.svg",
        href: "/dashboard/subs-manage",
        paymentRelated: true,
      },
      {
        label: "Cancel Subscription",
        icon: closeOutline,
        href: "/dashboard/cancel-sub",
        paymentRelated: true,
      },
    ],
  },
...
function SettingIcon({ icon }: { icon: string }) {
  if (icon === "/logo.svg")
    return (
      <img
        className="dashboard-setting-logo"
        src={icon}
        alt=""
        aria-hidden="true"
      />
...
function statusClass(status: string | null | undefined) {
  return `dashboard-subscription-status-${normalizedSubscriptionStatus(status) || "unknown"}`;
}
...
export default function DashboardPage() {
  ...
  const cachedSubscription = getSessionSubscription();
  const [user, setUser] = useState<CurrentUser | null>(() => cachedUser);
  const [subscription, setSubscription] =
    useState<SubscriptionManagement | null>(() => cachedSubscription);
  const [subscriptionLoading, setSubscriptionLoading] = useState(
    () => !cachedSubscription && cachedUser?.plan?.toLowerCase() !== "starter",
  );
  useEffect(() => {
    let mounted = true;
    ...
    async function loadDashboard() {
      try {
        ...
        setUser(freshUser);
        if (freshUser.plan?.toLowerCase() === "starter") {
          setSubscription(null);
          setSubscriptionLoading(false);
          return;
        }
        try {
          const freshSubscription = await Promise.race([
            getSubscriptionManagement(),
            new Promise<SubscriptionManagement>((_, reject) =>
              setTimeout(
          ...
          if (!mounted) return;
          cacheSubscriptionManagement(freshSubscription);
          setSubscription(freshSubscription);
        } catch (error) {
          console.error("Failed to load subscription management:", error);
        } finally {
          if (mounted) setSubscriptionLoading(false);
        }
      } catch (error) {
  ...
  const plan = formatPlan(subscription?.plan || user?.plan || "Starter");
  const endDate = subscription
    ? formatDate(subscription.subscription_ends_at)
    : formatDate(user?.subscription_ends_at ?? null);
  ...
  const normalizedStatus = normalizedSubscriptionStatus(subscriptionStatus);
  const isNonRenewing = ["non-renewing", "canceled", "cancelled"].includes(
    normalizedStatus,
  );
  return (
      ...
      <section className="dashboard-content" aria-label="Dashboard">
        <div className="dashboard-overview">
          <div className="dashboard-plan-card">
            ...
            {subscription && (
              <span
                className={`dashboard-subscription-status ${statusClass(subscriptionStatus)}`}
              >
            ...
            <div className="dashboard-plan-heading">
              ...
              {user || subscription ? (
                ...
                <span
                  className="dashboard-plan-spinner"
                  role="status"
            ...
            {subscription?.type === "one_time" ? (
              ...
              <p className="dashboard-billing-copy">
                ...
                {subscription?.interval
                  ? ` Billed ${subscription.interval == "month" ? "Monthly" : "Yearly"}.`
                  : ""}
              </p>
            )}
            {subscription && (
              <div className="dashboard-card-details">
                <strong>{paymentCardType(subscription)}</strong>
                {paymentDetail(subscription) && (
                  <span>{paymentDetail(subscription)}</span>
                )}
                {subscription.payment_method_details?.bank && (
                  <small>
                    {humanize(subscription.payment_method_details.bank)}
                  </small>
                )}
                ...
                {subscriptionStatus && (
                  <>
                    {
                      subscription.payment_method == "cancel"
  ...
  Card cancelled
                        ...
                        // ? <small className="dashboard-renewal-status"> Cancelled</small>
                        : ""
                    } 
                  </>
                )}
            ...
            {isNonRenewing && (
              <div className="dashboard-subscription-alert">
                <IonIcon icon={closeOutline} aria-hidden="true" />
                <span>
                  Cancelled — your access remains available until{" "}
                  {endDate ?? "the end of your current period"}.
                </span>
              </div>
            )}
          </div>
          ...
          <section className="dashboard-section-card" key={group.title}>
            ...
            <div className="dashboard-setting-list">
              {group.items.map((item) => {
                const disabled =
                  item.paymentRelated && (subscriptionLoading || isStarter);
                return (
                  <button
                    type="button"
                    className={`dashboard-setting-row${disabled ? " dashboard-setting-row-disabled" : ""}`}
                    key={item.label}
                    disabled={disabled}
                    aria-disabled={disabled}
                    onClick={() =>
                      !disabled && item.href && router.push(item.href)
                    }
                  >
                    <span className="dashboard-setting-label">
                      <SettingIcon icon={item.icon} />
                      <span>{item.label}</span>
                    </span>
                    <IonIcon
                      className="dashboard-setting-caret"
        ...
        <section className="dashboard-section-card dashboard-help-card">
          ...
          <button
            type="button"
            className="dashboard-setting-row dashboard-help-row"
          >
            <span className="dashboard-setting-label">
              <IonIcon icon={helpCircleOutline} aria-hidden="true" />
              <span>Echostream Support</span>
            </span>
            <IonIcon
              className="dashboard-setting-caret"
              icon={chevronForwardOutline}
