...
function formatSubscriptionType(type: string | null | undefined) { if (!type) return ""; return type === "one_time" ? "One time" : humanize(type); }
function formatSubscriptionStatus(status: string | null | undefined) { return humanize(status) || "Unknown"; }
function statusClass(status: string | null | undefined) { return `dashboard-subscription-status-${(status || "unknown").toLowerCase().replace(/[_\s]+/g, "-")}`; }
function paymentDetail(subscription: SubscriptionManagement) {
  ...
  if (details.bank) return humanize(details.bank);
  return "";
}
