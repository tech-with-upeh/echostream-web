...
function ExpiringSubscriptionNotice({ oneTime }: { oneTime: boolean }) {
  return (
    <div className="subs-expiring-notice">
      <img src="/noreoccuring.svg" className="h-1/6 w-1/4" alt="No Reoccuring Payment" />
      <p>
        {oneTime
...
export default function SubscriptionManagePage() {
  ...
  const [subscription, setSubscription] =
    useState<SubscriptionManagement | null>(() => getSessionSubscription());
  ...
  const managementLink =
    subscription?.management_link || subscription?.link || null;
  const paymentName =
    details?.brand ||
    details?.card_type ||
    details?.method ||
  ...
  const openManagementLink = () => {
    if (managementLink)
      window.open(managementLink, "_blank", "noopener,noreferrer");
  };
        ...
        <section className="subs-plan-card">
          <div className="subs-plan-top">
            ...
            <p className="subs-plan-description">
              Your current plan gives you everything you need for your streams,
              including premium voices and enhanced features.
            </p>
          </div>
          <div className="subs-plan-bottom">
            <div>
              ...
              <strong>
                {loading ? "" : isOneTime ? "One time" : interval}
              </strong>
              <small>{loading ? "" : endDate}</small>
            </div>
            <button
              type="button"
              className="subs-change-plan"
          ...
          <>
            <section className="subs-cancel-section">
              <div>
                ...
                <p>
                  Cancel your {plan} plan. You’ll continue to have access until
                  the end of your current billing period.
                </p>
              </div>
              <button
                type="button"
                className={`subs-cancel-button${!canCancel || !managementLink ? " subs-cancel-disabled" : ""}`}
                disabled={!canCancel || !managementLink}
                onClick={openManagementLink}
              >
                Cancel subscription
              </button>
            </section>
                  ...
                  <strong>
                    {loading
                      ...
                      : hasLast4
                        ? `•••• ${details?.last4}`
                        : details?.bank
                          ? humanize(details.bank)
                          : `${interval} billing`}
                  </strong>
                  {!loading && details?.bank && hasLast4 && (
                    <small>{humanize(details.bank)}</small>
                  )}
                  {!loading && details?.card_type && !details?.brand && (
                    <small>{humanize(details.card_type)}</small>
                  )}
                </div>
                <button
                  type="button"
                  className="subs-update-card"
                  disabled={loading || !managementLink}
                  aria-disabled={loading || !managementLink}
                  onClick={openManagementLink}
                >
                  Update
                </button>
