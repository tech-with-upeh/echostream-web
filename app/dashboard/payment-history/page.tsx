...
const formatMethod = (item: PaymentHistoryItem) =>
  `${humanize(item.billing_type)} · ${humanize(item.method || item.channel)}`;
      ...
      <section className="payment-history-content">
        ...
        <section className="payment-history-card">
          ...
          <div className="payment-list">
           {loading && (
  <div className="flex flex-col items-center justify-center p-6 space-y-3 payment-history-state">
    {/* Spinner Wheel */}
    <div className="w-8 h-8 border-4 border-gray-200 border-t-[#00EEFC] rounded-full animate-spin"></div>
              ...
              payments.map((payment) => (
                <article className="payment-row" key={payment.id}>
                  <div className="payment-row-main">
                    <div
                      className={`payment-status-icon payment-status-icon-${payment.status.toLowerCase().replaceAll(" ", "-")}`}
                    >
                      <IonIcon icon={statusIcon(payment.status)} />
                    </div>
