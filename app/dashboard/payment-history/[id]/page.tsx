...
export default function PaymentReceiptPage() {
  ...
  const title = useMemo(
    () => (payment ? `${formatPlan(payment.plan)} plan` : "Payment receipt"),
    [payment],
  );
  const copyPaymentId = async () => {
    if (!payment?.payment_id) return;
    try {
      await navigator.clipboard.writeText(payment.payment_id);
      setCopied(true);
    ...
    return (
      <main className="payment-history-page">
        ...
        <section className="receipt-content">
          ...
          <div className="receipt-state">Loading receipt…</div>
        </section>
      </main>
    );
  if (error || !receipt || !payment)
    return (
      <main className="payment-history-page">
        <DashboardNavbar />
        <section className="receipt-content">
          ...
          <div className="receipt-state">
            ...
            <p>{error ?? "We couldn't find this payment."}</p>
            <Link
              href="/dashboard/payment-history"
      ...
      <section className="receipt-content">
        ...
        <article className="receipt-card">
          <header className="receipt-card-header">
            ...
            <div className={`receipt-paid-badge ${paid ? "is-paid" : ""}`}>
          ...
          <section className="receipt-details">
            ...
            <div className="receipt-detail-row receipt-detail-copy">
              <span>EchoStream payment ID</span>
              <strong>{payment.payment_id}</strong>
              <button
                type="button"
                onClick={copyPaymentId}
                aria-label="Copy EchoStream payment ID"
              >
            ...
            <div className="receipt-detail-row">
              ...
              <strong>{humanize(payment.provider)}</strong>
            </div>
            <div className="receipt-detail-row">
              <span>Billing type</span>
              <strong>{humanize(payment.billing_type)}</strong>
            </div>
            <div className="receipt-detail-row">
              <span>Payment method</span>
              <strong>{humanize(payment.method)}</strong>
            </div>
            <div className="receipt-detail-row">
          ...
          {(subscription?.starts_at || subscription?.ends_at) && (
            <section className="receipt-details">
              ...
              <div className="receipt-detail-row">
                <span>Starts</span>
                <strong>{formatDate(subscription.starts_at)}</strong>
              </div>
              <div className="receipt-detail-row">
                <span>Ends</span>
