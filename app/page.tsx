...
export default function Home() {
  ...
  const [audioInfo, setAudioInfo] = useState(
    testimonials.map(() => ({ duration: 0, waveform: FALLBACK_WAVEFORM })),
  );
        ...
        try {
          ...
          try {
            ...
            const samplesPerBar = Math.max(
              1,
              Math.floor(channel.length / bars),
            );
            ...
            setAudioInfo((current) =>
              current.map((item, itemIndex) =>
                itemIndex === index
                  ? { duration: decoded.duration, waveform: peaks }
                  : item,
              ),
            );
          } finally {
      ...
      <section id="testimonials" className="testimonials-section">
        <div className="testimonials-inner">
          {testimonials.map((testimonial, index) => {
            ...
            const progress =
              duration > 0 && isActive ? currentTime / duration : 0;
            return (
              <article
              ...
              >
                ...
                <div className="testimonial-audio">
                  <button
                    ...
                    type="button"
                    onClick={() => toggleTestimonialAudio(index)}
                    aria-label={`${isActive ? "Pause" : "Play"} ${testimonial.name} testimonial`}
                  >
                    <IonIcon
                  ...
                  <div className="testimonial-waveform" aria-hidden="true">
                    {waveform.map((height, barIndex) => {
                      const played =
                        isActive &&
                        (barIndex + 1) / waveform.length <= progress;
                      return (
                        <span
                  ...
                  <span className="testimonial-duration">
                    {formatTime(isActive ? currentTime : duration)}
                  </span>
                </div>
              </article>
            );
